...
for frac in [1000, 5000, 10000, 15000, 17000]:
	...
	X_prcm_train, y_train = des_train.generatePRCM(y_train,numRep=4)
	
	# Generating the descriptor for the test set
	des_test = CoulombMatrix.CoulombMatrix(matrixX=X_test)
	X_prcm_test, y_test = des_test.generatePRCM(y_test,numRep=4)
	...
	print "For the training with " + str(frac) + " data points:"
